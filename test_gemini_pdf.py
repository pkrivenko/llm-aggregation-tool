"""Test Gemini with raw PDF upload (temperature=0.3 to avoid Gemini bug)"""
import asyncio
import base64
import re
...
# Parse questions and answers (handle end of file)
...
def load_pdf_raw() -> DocInfo:
    """Load PDF as raw base64 (no text extraction, no images)."""
    return DocInfo(
        doc_id="doc2.pdf",
        filename="doc2.pdf",
...
async def main():
    print("=" * 80)
    print("GEMINI RAW PDF TEST (temperature=0.3)")
    print(f"Questions: {len(QUESTIONS)} | Model: gemini-3-flash-preview")
    print("=" * 80)

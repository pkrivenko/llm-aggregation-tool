"""Benchmark PDF modes with Gemini 3 Flash - 10 iterations each, parallel execution"""
import asyncio
import base64
import time
...
async def run_single(mode_name: str, pdf_modes: list, iteration: int) -> RunResult:
    ...
    config = RunConfig(
        questions=[QUESTION],
        docs=[doc],
        doers=[ModelRow(model_id="google/gemini-3-flash-preview", timeout_s=60.0, n_calls=1)],
        judges=[],
        final_judges=[],
...
async def main():
    print("=" * 70)
    print("PDF Modes Benchmark - Gemini 3 Flash")
